    ...
    def post(self,request):
        ...
        for j in supplier_dict:
            ...
            if supplier_dict[j] != 0:
                ...
                if all_zeros:
                    supp_array[index] = 0
        relation_obj, created = supplier_Relationship_table.objects.update_or_create(
        admin_ID=data['admin_ID'],
        simulation_id=data['simulation_id'],
        defaults={'Relation_Object': {data['firm_id']: supp_array}}
    )
        if not created:
            relation_obj.Relation_Object = {data['firm_id']:supp_array}
            relation_obj.save()
        startup_cost = 30000
        subsequent_month_cost = 5000

        if created:
            startup_costs = [startup_cost if order_given != 0 else 0 for order_given in supp_array]
        else:
            last_month_suppliers = supplier_Relationship_table.objects.filter(
            admin_ID=data['admin_ID'],
            simulation_id=data['simulation_id'],
        ).values(*[f'Relation_Object__{i}' for i in range(1, 8)])

        startup_costs = []
        for index, quantity in enumerate(last_month_suppliers, 1):
            if quantity[f'Relation_Object__{index}'] == 0 and supp_array[index - 1] != 0:
                startup_costs.append(startup_cost)
            else:
                startup_costs.append(0)

        startup_costs += [0] * (len(supp_array) - len(startup_costs))
        on_going_costs = subsequent_month_cost
        fc_table_obj, created_fc = procurements_fc_table.objects.update_or_create(
            admin_ID=data['admin_ID'],
            simulation_id=data['simulation_id'],
            defaults={
                'supplier_A_FC': 0 if startup_costs[0] == 0 else startup_costs[0] + on_going_costs,
                'supplier_B_FC': 0 if startup_costs[1] == 0 else startup_costs[1] + on_going_costs,
                'supplier_C_FC': 0 if startup_costs[2] == 0 else startup_costs[2] + on_going_costs,
                'supplier_D_FC': 0 if startup_costs[3] == 0 else startup_costs[3] + on_going_costs,
                'supplier_E_FC': 0 if startup_costs[4] == 0 else startup_costs[4] + on_going_costs,
                'supplier_F_FC': 0 if startup_costs[5] == 0 else startup_costs[5] + on_going_costs,
                'supplier_G_FC': 0 if startup_costs[6] == 0 else startup_costs[6] + on_going_costs,
            }
        )
        fc_table_obj.save()
